...
const AllTxnList = () => {
    ...
    const getCustomerData = async () => {
        try {
            const req = await apiRequest(`app/customerList?page=${currentPage}&size=${pageSize}`);
            ...
            if (req?.data?.pageData) {
                const formattedCustomers: OptionType[] = Array.from(
                    new Map<string, OptionType>(
                        req.data.pageData.map((item: any) => [item.name, { value: item.name, label: item.name }])
                    ).values()
                );
    ...
    const handleViewTxn = (row: any) => {
        navigate("/merchant-transaction-details", {
            state: { id: row?.transactionId, path: '/merchants/transactions' },
        });
    };
        ...
        <div className="rounded-lg p-4 md:p-6 xl:p-6 lg:p-6">
            ...
            <div className="grid grid-cols-12 gap-4 mb-6">
                ...
                <div className="col-span-12 md:col-span-3">
                    <div className="relative">
                        <DatePicker
                            ...
                            maxDate={new Date()}
                            className="border w-full border-gray-300 text-gray-600 text-sm py-[10px] px-4 rounded focus:outline-none"
                            showIcon={false}
                        />
                        {selectedDate && (
                ...
                {/* <div className="col-span-12 md:col-span-2 flex items-end">
                    <button
                        type="button"
                        className="py-2 px-4 bg-green-600 text-white rounded-md w-full"
                        onClick={filterData}
                    >
                        Filter
                    </button>
            ...
            {!loader && (
                <>
                    <div className="grid grid-cols-1">
                        <Table columns={columnHeader} data={merData.pageData} viewDetails={undefined} onDelete={undefined} viewTxn={handleViewTxn} viewImage={undefined}
                            onEdit={undefined} />
                    </div>
